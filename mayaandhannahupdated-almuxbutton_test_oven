...
; ALT 
;check_Push_Button MAC ; new one with multiplexed buttons
    ; to be tested once the base function works
...
ORG 0x0000 ; Reset vector
        ljmp main_program_extend ;see line 226
...
PB_STOP_PIN         : dbit 1
FLAG_THAT_IS_1      : dbit 1
; Will be set to 1 'no push button pressed in main code'
...
; Multiplexed Pushbutton Logic ;
check_Push_Button:
        ; input pin used to check set to '1'
        setb SHARED_PIN
        ...
        setb INC_TEMP_PIN ; jump to checkTempInc if pressed
        setb INC_TIME_PIN ; jump to checkTimeInc if pressed
        setb STOP_PIN ; jump to STOP_PROCESS if pressed
        
        ; Check individual push buttons
        ; P1.3
        clr INC_TIME_PIN
        mov c, SHARED_PIN
        ...
        LCD_PB_DONE:
            ; ret
;---------------------------------;
; Send a BCD number to PuTTYÂ Â Â Â Â  ;
;---------------------------------;
Send_BCD mac
	push    ar0
	mov     r0, %0
...
Initilize_All:
	...
	mov	P0M1,#0x00
	mov	P0M2,#0x00
	mov FLAG_THAT_IS_1, #0x01 ;from Maya to use compare flags macro
        ...
        ; â†“ â†“ â†“ â†“ â†“ â†“ â†“ â†“ â†“ â†“ â†“ SUS  â†“ â†“ â†“ â†“ â†“ â†“ â†“ â†“ â†“ â†“ â†“ â†“ â†“   
...
; REDO: configure_LCD_multiplexing - athina
; ensure that no button pressing before the fsm begins affects the fsm
configure_LCD_multiplexing:
    ; set all bariables to 1: 'no push button pressed'
    setb PB_START_PIN
    setb PB_STOP_PIN 
...
MENU_FSM: 
        ...
        mov     a, MENU_STATE 
        ljmp check_Push_Button ;(CHANGE_MENU_PIN, checkTimeInc) ; increments menu state
        inc     a
        mov     MENU_STATE, a 
        setb    CHANGE_MENU_PIN
        ...
        checkTimeInc:
                setb CHANGE_MENU_PIN
                ljmp check_Push_Button;(INC_TIME_PIN, checkTempInc)
                setb INC_TIME_PIN
         ...
         checkTempInc:
                setb INC_TIME_PIN
                ljmp check_Push_Button ;(INC_TEMP_PIN, enterMenuStateCheck)
                setb INC_TEMP_PIN
...
main_program:
        ...
        checkStartButton: ; assumed negative logic - used a label for an easy ljmp in the future
                ljmp check_Push_Button;(START_PIN, noStartButtonPress)
                setb    START_PIN
